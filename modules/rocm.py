...
def get_version() -> str | None:
    try:
        ...
        print(f'ROCm: failed to get version: {e}')
        return None
...
if sys.platform == "win32":
    ...
    def rocm_init():
        try:
            ...
            agents = get_agents()
            print(f'ROCm: agents={agents}')
            if all(available.name not in build_targets for available in agents):
                print('ROCm: torch-rocm is installed, but none of build targets are available')
                # use cpu instead of crashing
                torch.cuda.is_available = lambda: False

            agent = get_hip_agent()
            print(f'ROCm: selected={agents}')
            if not agent.blaslt_supported:
                print(f'ROCm: hipBLASLt unavailable agent={agent}')
